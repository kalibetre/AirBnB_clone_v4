            ...
            data.map((place) => {
                places.append(`
                    <article>   
                        ...
                        <div id="owner_${place['id']}" class="user"></div>
                        <div class="description">${place['description']}</div>
                        <div id="amenity_${place['id']}" class="amenities"></div>
                        <div id="reviews_${place['id']}" class="reviews"></div>
                    </article>
                `);

                getUser(place['user_id'], (user) => {
...
function fetchReviews(place) {
    $.get(
        `http://0.0.0.0:5001/api/v1/places/${place['id']}/reviews`,
        (data) => {
            reviews = $(`#reviews_${place['id']}`);
            text = `<h2>${data.length} Reviews <span id='review_${place['id']}' class='review_toggle'>Show</span></h2>`;
            text += `<ul id='review_detail_${place['id']}'`;
            data.map((review) => {
                date = getFormattedDate(new Date(review['updated_at']));
                text += `
                    <li>
                        <h3 id='reviewer_${review['id']}'></h3>
                        <p>${review['text']}</p>
                    </li>   
                `;
            });
            ...
            data.forEach((review) => {
                getUser(review['user_id'], (user) => {
                    $(`#reviewer_${review['id']}`).append(`
                        From ${user['first_name']} the ${date}
                    `);
                });
            });

            $(`#review_detail_${place['id']}`).css({
                display: 'none',
            });

            $(`#review_${place['id']}`).click(function () {
...
function getFormattedDate(date) {
    ...
    return `${date.getDate()}th ${month} ${year}`;
}

let userCache = {};
function getUser(userId, callback) {
    if (userId in userCache) callback(userCache[userId]);
    else
        $.get(`http://0.0.0.0:5001/api/v1/users/${userId}`, (user) => {
